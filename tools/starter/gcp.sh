...
function usage {
  echo "Usage: ./starter.sh -t twitterLogin -p projectId"
  exit 1
}

if [[ -z "$twitterId" || -z "$projectId" ]]; then
  usage
fi

function cloneTemplate {
  echo -n "* Cloning template project..."
  RES=$(git clone "https://github.com/$twitterId/actions-on-google-project-template-gcp.git" actions-on-google 2>&1)
  checkResult "$RES"
}
